...
def chooseDropDown(driver, Xpath, value):
#    time.sleep(1)
...
def scroll_page_down(driver,controlxpath):
    controlClick=driver.find_element_by_xpath(controlxpath)
    controlClick.send_keys(Keys.PAGE_DOWN)
    controlClick.send_keys('\ue00f')
...
def processingCheckWait(driver,controlxpath, timeout):
    try:
        WebDriverWait(driver, h9 ).until(EC.presence_of_element_located((By.XPATH, controlxpath)))
